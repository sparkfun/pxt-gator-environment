...
bool BME280::beginI2C(MicroBitI2C &wirePort)
{
	_hardPort = &wirePort;
	_wireType = HARD_WIRE;
...
{
	uBit.i2c.readRegister(settings.I2CAddress, offset, outputPointer, length);
}

uint8_t BME280::readRegister(uint8_t offset)
{
	return uBit.i2c.readRegister(settings.I2CAddress, offset);
}
...
{
	uBit.i2c.writeRegister(settings.I2CAddress, offset, dataToWrite);
}
